...
class Events:
    ...
    async def events(self, ctx):
        if ctx.invoked_subcommand is None:
            ...
            embed.set_footer(text='[Generated by: {0}]'.format(ctx.author.name))
        ...
        with conn:
            for event_id in event_list:
                ...
                if 1 in c.fetchone():
                    ...
                    embed.add_field(name='{} Not Found'.format(event_id),
                                    value='\u200b',
                                    inline=False)
            await ctx.send(embed=embed)
        ...
        if eid == 'check':
            ...
            if not event_exists:
                ...
                with conn:
                    ...
                    if 1 in c.fetchone():
                        c.execute("""SELECT EXISTS(SELECT 1 FROM event_notify WHERE event_id = (:event_id) 
                         AND channel_id = (:channel_id))""", {'event_id': event_id, 'channel_id': cid})
                        if 1 in c.fetchone():
                            msg = 'This notification already exists.'
                        else:
                            c.execute("""UPDATE event_notify SET channel_id = (:channel_id) 
                            WHERE event_id = (:event_id)""", {'channel_id': cid, 'event_id': event_id})
                            fmt = (title, event_id, channel)
                            msg = 'The notification channel for event "{} ({})"\n' \
                                  'will send a notification in {.mention}.'.format(*fmt)
                    else:
                        c.execute("""
                            INSERT INTO event_notify 
                            VALUES(:event_id, :member_id, :guild_id, :channel_id, :datetime, :title)""",
                                  {'event_id': event_id, 'member_id': ctx.author.id,
                                   'guild_id': guild_id, 'channel_id': cid, 'datetime': dt,
                                   'title': title})
                        fmt = (ctx.author.name, channel, title)
                        msg = 'Set to notify {0} in {1.mention} when "{2}" is 1 hour away from starting!'.format(*fmt)
                    embed = discord.Embed(title='Notification Update', color=discord.Color.dark_purple(),
                                          description=msg)
